...
impl Db{
    ...
    fn add_node_definition(&mut self, new_node_definition: NodeDefinition) -> () {
        let name = new_node_definition.name.clone();
        let node_group = NodeGroup{
            definition: new_node_definition,
            current_id: 0,
            nodes: BTreeMap::new()
        };
        self.node_groups.insert(name, node_group);
    }

    fn remove_node_definition(&mut self, node_definition_name: String) -> () {
        self.node_groups.remove(&node_definition_name);
    }
    ...
    fn add_node(&mut self, node_type: String, new_node: Node) -> u64 {
        let mut id : u64;
        let group = self.node_groups
                    .get(&node_type)
                    .map(|group| {
                        id = group.current_id;
                        group.nodes.insert(id, new_node);
                        group.current_id = group.current_id + 1;
                    });
...
mod tests {
    ...
    fn test_remove_node_definition() {
        ...
        node_groups.insert(String::from("Test Node Name"), node_group);
